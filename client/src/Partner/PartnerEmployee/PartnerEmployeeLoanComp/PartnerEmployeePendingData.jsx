...
export default function PartnerEmployeePendingData() {
  ...
  const fetchAlldata = async () => {
    ...
    let data = await fetch(
      `http://localhost:5000/api/p_getpendingdatas/${email}?Status=${Status}&filter=${selectedFilter}&bankFilter=${selectedBankFilter}`,
      {
        method: "GET",
        credentials: "include",
  ...
  const handleDelete = async (_id) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/p_deletePendingData/${_id}`,
        {
          method: "DELETE",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  const handleSubmit = () => {
    if (selectedData && formType === "Approved") {
      ...
      axios
        .post(
          "http://localhost:5000/api/p_submit-approved-data",
          approvedData,
          {
            headers: {
  ...
  const handleRejectSubmit = () => {
    if (selectedData && formType === "Rejected") {
      ...
      // Make a POST request to your backend API
      axios
        .post("http://localhost:5000/api/p_submit-rejct-data", approvedData, {
          headers: {
            "Content-Type": "application/json",
          },
        })
