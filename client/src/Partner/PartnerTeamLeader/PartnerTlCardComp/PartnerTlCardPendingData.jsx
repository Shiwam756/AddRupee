...
export default function PartnerTlCardPendingData() {
  ...
  const handleDelete = async (_id) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/p_card_deletePendingData/${_id}`,
        {
          method: "DELETE",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  const handleTlDelete = async (_id) => {
    try {
      // Send a DELETE request to the backend to delete the pending data
      const response = await fetch(
        `http://localhost:5000/api/p_card_deletetlPendingData/${_id}`,
        {
          method: "DELETE",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  const handleSubmit = () => {
    if (selectedData && formType === "Approved") {
      ...
      axios
        .post(
          "http://localhost:5000/api/p_card_submit-approved-data",
          approvedData,
          {
            headers: {
  ...
  const handleRejectSubmit = () => {
    if (selectedData && formType === "Rejected") {
      ...
      axios
        .post(
          "http://localhost:5000/api/p_card_submit-rejct-data",
          approvedData,
          {
            headers: {
              "Content-Type": "application/json",
