  ...
  const handleUpload = async (e) => {
    ...
    try {
      const response = await Axios.post(
        "http://localhost:5000/api/p_import-csv",
        formData,
        {
          headers: {
    ...
    const fetchTeamDetails = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/p_getteamdetails/${Tl_Name}`
        );
...
export default PartnerTlUploadCSV;
